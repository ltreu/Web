...
if (isset($_POST['hidden_data']) && $_POST['hidden_data'] != NULL && isset($_POST['hidden_data2']) && $_POST['hidden_data2'] != NULL)
{
	...
	imagepng($img, "../../img/gallery/".$filename);


	$file = $upload_dir . $filename;
	$path = $file;
	$path_for_bdd = "img/gallery/" . $filename;
	$id = $_SESSION['id'];
	try{
		$date_upload = time();
		...
		$request = $bdd->prepare("INSERT INTO `photos` (`link`, `id_user`, `date_upload`)
		VALUES(:link, :id_user, :date_upload)");
		$request->bindParam(':link', $path_for_bdd);
